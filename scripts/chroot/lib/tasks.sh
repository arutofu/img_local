...
# -----------------------------
# Drone deps only (for middle.img)
# -----------------------------
install_drone_build_deps_only() {
  info "Install deps for drone build (without building workspace)"
  [[ -f "/opt/ros/${ROS_DISTRO}/setup.bash" ]] || die "ROS required for deps install, missing /opt/ros/${ROS_DISTRO}/setup.bash"
  ...
  apt_install \
    ...
    libopencv-dev || {
      apt_policy_dump
      die "Failed to install deps for drone build"
    }
}
